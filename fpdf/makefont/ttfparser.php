...
class TTFParser
{
	...
	function ParseHead()
	{
		...
		$this->index_1ToLocFormat = $this->ReadShort();
	}
	...
	function ParseOS2()
	{
		...
		$this->Skip(2*2); // usFirstCharindex_1, usLastCharindex_1
	...
	function ParsePost()
	{
		...
		if($version==0x20000)
		{
			...
			$glyphNameindex_1 = array();
			$names = array();
			$numNames = 0;
			for($i=0;$i<$this->numGlyphs;$i++)
			{
				$index_1 = $this->ReadUShort();
				$glyphNameindex_1[] = $index_1;
				if($index_1>=258 && $index_1-257>$numNames)
					$numNames = $index_1-257;
			}
			...
			foreach($glyphNameindex_1 as $i=>$index_1)
			{
				if($index_1>=258)
					$this->glyphs[$i]['name'] = $names[$index_1-258];
				else
					$this->glyphs[$i]['name'] = $index_1;
			}
